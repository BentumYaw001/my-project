...
function Preview({ markdown, toggleEditorVisibility, isEditorVisible, isDarkTheme }) {
  const parsed = marked(markdown);

  return (
    <div className={`w-full flex flex-col ${isEditorVisible ? '' : 'h-full items-center'}`}>
      <div className={`p-3 uppercase tracking-wider justify-between flex items-center w-full ${isDarkTheme ? 'bg-customblack1 text-gray-400' : 'bg-gray-200 text-gray-800'}`}>
        Preview
        <button
          onClick={toggleEditorVisibility}
          id={isEditorVisible ? "HidePreview" : "ShowPreview"}
          className="group flex-col justify-between float-end mr-5 p-1"
        >
          <svg
            width="16"
            height={isEditorVisible ? "12" : "15"}
            xmlns="http://www.w3.org/2000/svg"
            className={`fill-current ${isDarkTheme ? 'text-gray-500 group-hover:text-customRed' : 'text-gray-700 group-hover:text-red-700'}`}
          >
            {isEditorVisible ? (
      ...
      <div className={`w-full flex-grow prose  p-6 ${isDarkTheme ? 'prose-invert text-white bg-mainblack ' : 'prose bg-white text-black'} ${isEditorVisible ? '' : 'w-full'}`} dangerouslySetInnerHTML={{ __html: parsed }} />
    </div>
  );
}
